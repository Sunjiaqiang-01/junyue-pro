...
// Define ChartData interface locally since it's not exported by reaviz
interface ChartData {
  key: string;
  data: number;
}
...
const DiamondAlertIcon: React.FC<{ className?: string; fill?: string }> = ({
  className,
  fill = "#E84045",
}) => (
  <svg
...
const CircleAlertIcon: React.FC<{ className?: string; fill?: string }> = ({
  className,
  fill = "#E84045",
}) => (
  <svg
...
const TrendUpIcon: React.FC<{ baseColor: string; strokeColor: string; className?: string }> = ({
  baseColor,
  strokeColor,
  className,
}) => (
...
const TrendDownIcon: React.FC<{ baseColor: string; strokeColor: string; className?: string }> = ({
  baseColor,
  strokeColor,
  className,
}) => (
  ...
  return (
    <>
      ...
      <div className="flex flex-col pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-full max-w-md min-h-[580px] overflow-hidden transition-colors duration-300">
        <h3 className="text-3xl text-left p-7 pt-6 pb-8 font-bold text-text-primary dark:text-white transition-colors duration-300">
          Incident Report
        </h3>

        <div className={"flex-grow px-4 reaviz-chart-container"}>
          {" "}
          {/* flex-grow to allow chart to take space */}
          <BarChart
                ...
                tickSeries={
                  <LinearYAxisTickSeries
                    label={
                      <LinearYAxisTickLabel
                        format={(text) =>
                          typeof text === "string" ? `${text.slice(0, 5)}...` : ""
                        }
          ...
          {METRICS_DATA.map((metric) => (
            ...
            >
              ...
              <div className="flex gap-2 w-1/2 justify-end items-center">
                <span className="font-semibold text-xl text-text-primary dark:text-white transition-colors duration-300">
                  {metric.value}
                </span>
                <metric.TrendIcon
