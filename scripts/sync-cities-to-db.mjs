...
const chinaCities = [
  ...
  { value: "克孜勒苏柯尔克孜自治州", label: "克孜勒苏柯尔克孜自治州", province: "新疆维吾尔自治区" },
...
function generateCode(cityName) {
  const pinyinMap = {
    ...
    '邯郸': 'handan', '邢台': 'xingtai', '保定': 'baoding', '张家口': 'zhangjiakou',
    '承德': 'chengde', '沧州': 'cangzhou', '廊坊': 'langfang', '衡水': 'hengshui',
    ...
    '南京': 'nanjing', '苏州': 'suzhou', '杭州': 'hangzhou', '郑州': 'zhengzhou',
    '广州': 'guangzhou', '深圳': 'shenzhen', '武汉': 'wuhan', '成都': 'chengdu',
    '西安': 'xian', '长沙': 'changsha',
    // 更多城市拼音映射...
  };
...
async function syncCities() {
  ...
  try {
    ...
    console.log(`📊 数据库现有城市: ${existingCities.length}个`);
    console.log(`📊 待导入城市: ${chinaCities.length}个\n`);
    ...
    for (const city of chinaCities) {
      if (existingCityNames.has(city.value)) {
        skippedCount++;
        console.log(`⏭️  跳过已存在: ${city.value}`);
        continue;
      }
      ...
      addedCount++;
      console.log(`✅ 添加成功: ${city.value}`);
    }
    
    console.log(`\n🎉 同步完成！`);
    console.log(`   新增: ${addedCount}个`);
    console.log(`   跳过: ${skippedCount}个`);
    console.log(`   总计: ${addedCount + existingCities.length}个城市`);
